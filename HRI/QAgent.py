...
# -*- coding: utf-8 -*-
"""
Created on Tue May 25 16:07:57 2021
...
class QAgent():
    ...
    def __init__(self, dungeon):
        ...
        self.rewards = []
        self.q_vals = np.zeros((config.worldLength, config.worldBreadth, 4))
        self.robLoc = self.gameWorld.getRobotLocation()
        self.storage = []
        self.agentsLoc = [] 
    ...
    def util_map(self):
        u_map = np.zeros((config.worldLength, config.worldBreadth))
        strawb = 1
        pits = -1
        human = -1
