...
class QAgent():
    ...
    def __init__(self, dungeon):
        ...
        self.agentLoc = self.gameWorld.getRobotLocation()
        # get map dims
        self.minimapX = config.worldLength-1
        self.minimapY = config.worldBreadth-1
    ...
    def build_map(self): # builds minimap for print to console then duplicates to statemap for computation
        ...
        # self.minimap.fill('s')
        # for Wumpus
        w = self.gameWorld.getHumanLocation() # this is list so loop
        # print(type(w))
        for i in w:
        ...
        # for Gold
        g = self.gameWorld.getStrawberryLocation() # this is list so loop
        # print(type(g))
        for i in g:
